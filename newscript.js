...
const triggerCount = new WeakMap();

function triggerTextDrop() {
  document.querySelectorAll(".drop-text1").forEach((el) => {
    let count = triggerCount.get(el) || 0;

    if (isInViewport(el)) {
      if (count === 0) {
        // First animation: play then disable
        el.classList.remove("in-view");
        void el.offsetWidth; // reflow
        el.classList.add("in-view");
        triggerCount.set(el, 1);
        ...
        // Second animation: play and KEEP class
        el.classList.remove("in-view");
        void el.offsetWidth;
        el.classList.add("in-view");
        triggerCount.set(el, 2); // lock at 2
      }
    }
  });
}
      ...
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          cover.style.animation =
            "slideIn 1.5s cubic-bezier(1, 0.2, 0.2, 1) forwards";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const observer = new IntersectionObserver(
    (entries, observer) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          const cover = entry.target.querySelector(".imganimationss");

          // img.style.animation = "scaleImg 1.5s cubic-bezier(1, 0.2, 0.2, 1) forwards";
          cover.style.animation =
            "slideInw 1.5s cubic-bezier(1, 0.2, 0.2, 1) forwards";
...
window.addEventListener("scroll", function () {
  document.querySelectorAll(".img-container").forEach((section) => {
    ...
    img.style.transform = `translateY(${progress * -150}px)`;
  });
});
    ...
    (entries, observer) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          img.style.animation =
            "scaleImg 1.5s cubic-bezier(1, 0.2, 0.2, 1) forwards";
